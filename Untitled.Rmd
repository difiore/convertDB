...
4.1 - A summary table of total avista


The narrative section for each part of the report should be minimized.  When an important event is witnessed a Word document with a brief, yet detailed, description of the event is required (for example a fight between two groups) and should be attached and referenced in the report.As you prepare each section you will have to relate your narrative part to the quantitative part. The function of the narrative part is also to summarize what you did that month.


2- A quantitative section with various tables summarizing data collected over the past month
...
Circle or highlight the corresponding important events:
...
d <- rowid_to_column(d, "rowID")
vars <- c("rowID", "Obs Sample ID", "Observer", "Date", "GPS Used", "Avistaje ID", "Taxon", "Group", "Time Enc", "Time Left/Lost", "Avistaje Notes", "Other Observer Present")
d <- select(d,vars)
...
output <- d %>% group_by(`Taxon`,`Group`,`Date`, `Observer`, `Other Observer Present`) %>% summarize(`Num Avistajes` = n(), AVDUR = sum(`Avistaje Duration`, na.rm = T))
output
